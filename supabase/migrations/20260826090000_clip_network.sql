...
-- Supabase service_role can still read/write it from Edge Functions.
...
-- User-visible tables: tenant-isolated CRUD.
do $$
declare t text;
begin
...
-- Bucket for rendered/test clips. Keep upload restricted by Storage RLS; public read is needed by
-- Instagram publishing. For production, front this with a controlled public media domain.
insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
values ('clip-network', 'clip-network', true, 1073741824, array['video/mp4','video/quicktime'])
