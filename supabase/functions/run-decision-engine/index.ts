    ...
    if (hasMinData) {
      ...
      if (
        ...
        items.push({
          ...
          headline: `Você está perdendo ${formatMoney(dailyWaste)}/dia aqui`,
          reason: `CTR no fundo do poço (${(ad.ctr * 100).toFixed(2)}%). Já rodou ${ad.days_active} dias. Está queimando dinheiro.`,
          impact_type: "waste",
          ...
          actions: [
            ...
            { id: uuid(), label: "Analisar gasto", type: "neutral", requires_confirmation: false },
          ],
      ...
      if (ad.conversions === 0 && ad.spend_cents > baseline.cpa_target_cents * 2) {
        ...
        items.push({
          ...
          headline: `Anúncio sugando ${formatMoney(dailyWaste)}/dia sem retorno`,
          reason: `Gastou ${formatMoney(ad.spend_cents)} e 0 conversões. Sangria pura.`,
          impact_type: "waste",
          impact_daily_cents: dailyWaste,
          impact_7d_cents: dailyWaste * 7,
          confidence: "high",
          ...
          actions: [
            ...
            { id: uuid(), label: "Revisar segmentação", type: "neutral", requires_confirmation: false },
          ],
      ...
      if (ad.cpa_cents > baseline.baseline_median_cpa * 2.5 && ad.conversions > 0) {
        ...
        items.push({
          ...
          headline: `CPA ${(ad.cpa_cents / baseline.baseline_median_cpa).toFixed(1)}x acima da linha`,
          reason: `Custa ${formatMoney(ad.cpa_cents)} por conversão. Mediana é ${formatMoney(baseline.baseline_median_cpa)}. Não sobrevive.`,
          impact_type: "waste",
      ...
      if (
        ...
        items.push({
          ...
          headline: "CPM subindo — público saturado",
          reason: `CPM subiu ${Math.round(((ad.cpm_cents / ad.prev_cpm_cents) - 1) * 100)}% e CPA está ${formatMoney(ad.cpa_cents)}. Segmentação precisa de ajuste.`,
          impact_type: "savings",
      ...
      if (ad.frequency > 3.0 && ad.cpa_cents > baseline.baseline_median_cpa * 1.3) {
        ...
        items.push({
          ...
          headline: `Desgaste já visível — frequência ${ad.frequency.toFixed(1)}x`,
          reason: `Cada pessoa viu ${ad.frequency.toFixed(1)}x em média. CPA subiu para ${formatMoney(ad.cpa_cents)}. Pausa alguns dias ou renova o criativo.`,
          impact_type: "savings",
      ...
      if (ad.has_hook && ad.hook_rate > 0.4 && ad.ctr < baseline.baseline_median_ctr) {
        ...
        items.push({
          ...
          reason: `Galera vê e aprecia (hook ${(ad.hook_rate * 100).toFixed(0)}%), mas não clica o suficiente. Mexe no CTA ou copy.`,
          impact_type: "savings",
      ...
      if (
        ...
        items.push({
          ...
          reason: `CTR ${(ad.ctr * 100).toFixed(2)}% está bom. Mas CPA ${formatMoney(ad.cpa_cents)} está caro. Problema no funil ou oferta.`,
          impact_type: "savings",
          ...
          metrics_snapshot: buildMetricsSnapshot(ad, baseline),
          actions: [
            { id: uuid(), label: "Revisar oferta", type: "neutral", requires_confirmation: false },
            { id: uuid(), label: "Ajustar segmentação", type: "constructive", requires_confirmation: false },
          ],
      ...
      if (ad.roas > baseline.baseline_median_roas * 2 && ad.conversions >= 2) {
        ...
        items.push({
          ...
          reason: `Esse anúncio tá voando. ROAS ${ad.roas.toFixed(2)}x. Mediana é ${baseline.baseline_median_roas.toFixed(2)}x. Aumenta budget em 50-100%.`,
          impact_type: "revenue",
      ...
      if (ad.cpa_cents < baseline.baseline_median_cpa * 0.7 && ad.conversions >= 2) {
        ...
        items.push({
          ...
          headline: `CPA ${formatMoney(ad.cpa_cents)} — ${savingsPct}% melhor que a média`,
  ...
  return [
    ...
    },
    {
      ...
      actions: [
        ...
        { id: "demo_2b", label: "Revisar segmentação", type: "neutral", requires_confirmation: false },
      ],
    ...
    },
    {
      ...
      actions: [
        { id: "demo_4a", label: "Revisar oferta", type: "neutral", requires_confirmation: false },
