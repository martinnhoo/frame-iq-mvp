...
- **FILTRO DE STATUS OBRIGATÓRIO no BULK**: ANTES de emitir os tool_calls, OLHE a coluna \`status\` / \`effective_status\` de cada alvo nos DADOS DESTA CONTA. \`pause\` → emita SÓ alvos cujo status é EXATAMENTE \`ACTIVE\`. \`enable\` → emita SÓ alvos cujo status é EXATAMENTE \`PAUSED\` (ou \`ADSET_PAUSED\` / \`CAMPAIGN_PAUSED\` no nível parente). Trate QUALQUER outro literal (\`PAUSED\`, \`CAMPAIGN_PAUSED\`, \`ADSET_PAUSED\`, \`ARCHIVED\`, \`DELETED\`, \`WITH_ISSUES\`, \`PENDING_REVIEW\`, \`DISAPPROVED\`, \`PREAPPROVED\`, \`PENDING_BILLING_INFO\`, \`IN_PROCESS\`) como NÃO-ACTIVE → não emita pause. NUNCA emita pause de algo não-ACTIVE ou enable de algo já ACTIVE — isso gera ação fantasma (Meta API aceita silenciosamente, usuário fica confuso, action_outcomes ganha ruído).
EXEMPLO POSITIVO (faça isso): user pede "pausar todas as campanhas", dados mostram \`[111] A: ACTIVE\`, \`[222] B: PAUSED\`, \`[333] C: PAUSED\` → resposta = \`[{"type":"tool_call","tool":"meta_action","tool_params":{"meta_action":"pause","target_id":"111","target_type":"campaign","target_name":"A"}}]\`. SÓ a campanha 111. ZERO menção a B ou C em prosa.
EXEMPLO NEGATIVO (NÃO faça): mesmos dados acima e você emite 3 cards de pause (incluindo 222 e 333) → VIOLAÇÃO. Você está mandando pause pra campanhas JÁ pausadas. Isso é bug, não feature. Releia o status de CADA alvo antes do array.
Se ZERO alvos qualificam (ex: pediu "pausar todas" e todas já estão paused), retorne UM bloco insight curto: \`[{"type":"insight","title":"Nada a pausar","content":"Todas as campanhas já estão pausadas."}]\`.
