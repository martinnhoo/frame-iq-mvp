...
- **FILTRO DE STATUS OBRIGATÓRIO no BULK**: ANTES de emitir os tool_calls, OLHE a coluna \`status\` / \`effective_status\` de cada alvo nos DADOS DESTA CONTA. \`pause\` → emita SÓ alvos com status \`ACTIVE\`. \`enable\` → emita SÓ alvos com status \`PAUSED\`. NUNCA emita pause de algo já PAUSED ou enable de algo já ACTIVE — isso gera ação fantasma (Meta API aceita silenciosamente, usuário fica confuso, action_outcomes ganha ruído). Se das 3 campanhas, só 1 está ACTIVE e o user pediu "pausar todas", emita UM array JSON com APENAS o tool_call dessa 1 + nada mais. NÃO mencione as outras pausadas em prosa pós-array (proibido por regra anterior). Se ZERO alvos qualificam (ex: pediu "pausar todas" e todas já estão paused), retorne UM bloco insight curto: \`[{"type":"insight","title":"Nada a pausar","content":"Todas as campanhas já estão pausadas."}]\`.
