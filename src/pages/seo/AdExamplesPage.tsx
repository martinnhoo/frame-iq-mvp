...
import { SEO_AD_EXAMPLES_PAGES } from "@/data/seoData";

export default function AdExamplesPage() {
  const { slug } = useParams<{ slug: string }>();
  const navigate = useNavigate();
  const page = SEO_AD_EXAMPLES_PAGES.find(p => p.slug === slug);
  ...
  return (
    <SeoLandingPage
      ...
      faqs={page.faqs}
      relatedLinks={[{ label: "Hook Generator", href: "/tools/ad-hook-generator" }, { label: "UGC Scripts", href: "/use-case/ugc-script-generator" }, { label: "Ad Creative Analyzer", href: "/tools/ad-creative-analyzer" }]}
      accentColor="#fb923c"
    />
  );
}
