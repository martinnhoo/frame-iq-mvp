      ...
      <div style={{ maxWidth: 1000, margin: "0 auto", padding: "64px 24px 0" }}>
        <div style={{ textAlign: "center", marginBottom: 56 }}>
          ...
          <h1 style={{ ...j, fontSize: 44, fontWeight: 800, letterSpacing: "-0.035em", lineHeight: 1.1, marginBottom: 14 }}>
            ...
            <span style={{ background: "linear-gradient(135deg,#a78bfa,#f472b6)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>ad creatives</span>
          </h1>
          ...
          {SEO_TOOLS.map(tool => (
            <div key={tool.slug} onClick={() => navigate(`/tools/${tool.slug}`)}
              ...
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 12 }}>
                <div style={{ width: 44, height: 44, borderRadius: 14, background: `${typeColor[tool.type] ?? "#a78bfa"}15`, border: `1px solid ${typeColor[tool.type] ?? "#a78bfa"}30`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>{tool.emoji}</div>
              ...
              <div style={{ fontSize: 12, color: "#a78bfa", fontWeight: 600 }}>Try it free →</div>
            </div>
          ))}
        </div>
        <SeoCTA />
