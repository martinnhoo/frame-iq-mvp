...
export default function SeoLandingPage({ metaTitle, metaDescription, canonical, badge, headline, subheadline, intro, useCases = [], faqs = [], relatedLinks = [], accentColor = "#a78bfa" }: SeoLandingPageProps) {
    ...
    <div style={{ minHeight: "100vh", background: "#080810", color: "rgba(255,255,255,0.85)" }}>
      ...
      <nav style={{ padding: "16px 24px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={() => navigate("/signup")} style={{ ...j, padding: "8px 16px", borderRadius: 999, fontSize: 13, fontWeight: 700, background: `linear-gradient(135deg,${accentColor},#f472b6)`, color: "#000", border: "none", cursor: "pointer" }}>Try free</button>
        </div>
      </nav>
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "64px 24px 80px" }}>
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 48 }}>
          <button onClick={() => navigate("/signup")} style={{ ...j, padding: "13px 28px", borderRadius: 999, fontSize: 14, fontWeight: 700, background: `linear-gradient(135deg,${accentColor},#f472b6)`, color: "#000", border: "none", cursor: "pointer" }}>Try free — no credit card</button>
        ...
        <div style={{ borderRadius: 20, padding: "32px 28px", textAlign: "center", background: "linear-gradient(135deg, rgba(139,92,246,0.1), rgba(236,72,153,0.05))", border: "1px solid rgba(139,92,246,0.15)" }}>
          ...
          <p style={{ color: "rgba(255,255,255,0.4)", marginBottom: 20, fontSize: 14 }}>Free plan includes 3 analyses. No credit card required.</p>
          <button onClick={() => navigate("/signup")} style={{ ...j, padding: "13px 28px", borderRadius: 999, fontSize: 14, fontWeight: 700, background: "linear-gradient(135deg,#a78bfa,#f472b6)", color: "#000", border: "none", cursor: "pointer" }}>Start free →</button>
        </div>
      </div>
    </div>
  );
