...
export default function ScriptGenerator() {
  ...
  const generate = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("generate-script", {
        body: { product, offer, audience, format, platform, market, duration, angle, extra_context: extraContext, user_id: user.id, persona_id: selectedPersona?.id || null },
      });
  ...
  return (
    <div className="tool-page-wrap page-enter max-w-4xl mx-auto space-y-6">
      ...
      <div className="rounded-2xl border border-border/50 p-6 space-y-4" style={{ background: "linear-gradient(145deg,rgba(255,255,255,0.055) 0%,rgba(255,255,255,0.015) 100%)", borderTopColor: "rgba(255,255,255,0.14)", boxShadow: "0 4px 20px rgba(0,0,0,0.45), inset 0 1px 0 rgba(255,255,255,0.06)" }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider" style={mono}>{language === "pt" ? "Oferta / CTA (opcional)" : language === "es" ? "Oferta / CTA (opcional)" : "Offer / CTA (optional)"}</label>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
          ...
          <div className="space-y-2">
            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider" style={mono}>{language === "pt" ? "Duração" : language === "es" ? "Duración" : "Duration"}</label>
