...
export default function HubStoryboard() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      // Quando user vai sobrepor logo via canvas (brand OU custom),
      // instrui o modelo a NÃO renderizar marca como elemento.
      if (effectiveLogoUrl && includeLogo) {
        const brandLabel = brand && brand.id !== "none" ? brand.name : "any logo";
        brandHint = `${brandHint}\n\nIMPORTANT: Do NOT render ${brandLabel} or any logo as text or visual element inside any scene. The official logo will be added as overlay in post-production. Keep the upper-right corner of every scene visually clean (about 20% area) so the overlay logo will be legible against any background.`;
      }
      ...
      // Persiste cenas no DB (frontend, RLS-safe). Bypassa qualquer
      // INSERT silencioso do edge function.
      try {
  ...
  return (
    <>
      ...
      <div style={{ minHeight: "calc(100vh - 0px)", padding: "20px 28px 64px", maxWidth: 1480, margin: "0 auto", color: "#fff" }}>
        ...
        {/* Logo controls: brand toggle + custom upload */}
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap", marginBottom: 8 }}>
              ...
              <label style={{ display: "inline-flex", alignItems: "center", gap: 8, cursor: "pointer", fontSize: 12 }}>
                ...
                <span style={{ color: "#fff", fontWeight: 600 }}>
                  {lang === "pt" ? "Incluir em todas as cenas" : lang === "es" ? "Incluir en todas las escenas" : lang === "zh" ? "在所有场景中包含" : "Include in every scene"}
                </span>
              </label>
            </div>
            <textarea
