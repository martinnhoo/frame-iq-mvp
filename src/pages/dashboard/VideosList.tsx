    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-white/50 text-sm mt-0.5">{videos.length} generated</p>
        </div>
      </div>

      {/* Generation panel */}
      {boardState?.boardId && (
        <div className="rounded-2xl border border-white/[0.1] bg-white/[0.07] p-6 space-y-4">
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              <span className="text-[10px] text-white/40 ml-1">{attemptsLeft} left</span>
            </div>
          </div>
          ...
          {genStatus === "done" && showFeedback && attemptsLeft > 0 && (
            <div className="space-y-3 border-t border-white/[0.12] pt-4">
              ...
              <textarea
                ...
                rows={2}
                className="w-full px-4 py-3 rounded-xl bg-white/[0.08] border border-white/[0.15] text-white placeholder:text-white/40 text-xs resize-none outline-none focus:border-white/20 transition-colors"
              />
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-xl text-white/50 text-xs hover:text-white/60 transition-colors"
                >
                  Keep this one
                </button>
              </div>
      ...
      {videos.length === 0 ? (
        <div className="rounded-2xl border border-white/[0.15] bg-white/[0.06] py-16 flex flex-col items-center gap-4 text-center">
          ...
          <div>
            ...
            <p className="text-white/45 text-sm mt-1">Open a board and click "Generate video from this board"</p>
          </div>
        </div>
      ) : (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {videos.map((video) => (
            <div
              key={video.id}
              className="group rounded-2xl border border-white/[0.15] bg-white/[0.06] overflow-hidden hover:border-white/20 transition-all"
            >
              <div className="aspect-video bg-white/[0.08] flex items-center justify-center relative">
              ...
              <div className="p-4">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <button
                    ...
                    className="opacity-0 group-hover:opacity-100 text-white/40 hover:text-red-400 transition-all shrink-0"
                  >
                    {deleting === video.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Trash2 className="h-3.5 w-3.5" />}
                  </button>
                </div>
                <p className="text-xs text-white/45 mt-1">{formatDistanceToNow(new Date(video.created_at), { addSuffix: true })}</p>
                {video.status === "completed" && video.video_url && (
                  <a
                    href={video.video_url}
                    download
