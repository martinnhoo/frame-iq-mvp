      ...
      <div className="page-enter p-4 sm:p-6 lg:p-8 max-w-2xl mx-auto space-y-6">
        ...
        {showForm ? (
          <div className="space-y-4">
            {/* Title */}
            <div>
              <label className="text-xs text-white/40 uppercase tracking-wider mb-2 block">Título (opcional)</label>
              <input value={title} onChange={e => handleTitleChange(e.target.value)} placeholder="ex: Nike Q1 — Teste UGC"
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="text-xs text-white/40 uppercase tracking-wider">Dados Meta Ads <span className="normal-case text-white/25">(opcional — enriquece a análise com CTR, ROAS, retenção reais)</span></label>
              </div>
              ...
              {!csvLoaded ? (
                <div
                  ...
                  <div className="flex items-center justify-center gap-2">
                    ...
                    <span className="text-xs text-white/30">Arraste seu CSV do Meta Ads aqui ou <span className="text-sky-400/70">procure</span></span>
                  </div>
                  <p className="text-[10px] text-white/15 mt-1">Exporte do Meta Ads Manager → Relatórios → Nível de anúncios → Exportar CSV</p>
                </div>
              ) : (
                <div className="rounded-2xl border border-white/[0.08] overflow-hidden">
                  ...
                  <div className="flex items-center gap-2 px-4 py-2.5" style={{ background: "rgba(14,165,233,0.06)", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                    ...
                    <span className="text-xs font-semibold text-sky-400">{csvRows.length} anúncios encontrados no CSV</span>
                  ...
                  {matchedRow ? (
                    <div className="px-4 py-3">
                      ...
                      <button onClick={() => setShowCsvPicker(!showCsvPicker)}
                        ...
                        Match errado? Selecione manualmente <ChevronDown size={11} />
                      </button>
                    </div>
                  ) : (
                    <div className="px-4 py-3">
                      <p className="text-xs text-white/40 mb-2">Nenhum match automático. Selecione o anúncio correto:</p>
                    </div>
                  )}
            ...
            <div className={`rounded-2xl border-2 border-dashed transition-all duration-200 cursor-pointer ${
              ...
              {file ? (
                <div className="flex items-center gap-4 p-5">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs mt-0.5" style={{ color: needsExtraction(file) ? "#fbbf24" : "rgba(255,255,255,0.3)" }}>
                      {(file.size / (1024 * 1024)).toFixed(1)} MB{needsExtraction(file) && " · Áudio será extraído automaticamente"}
                    </p>
                  </div>
                ...
                <div className="flex flex-col items-center gap-3 py-10 px-6">
                  ...
                  <p className="text-white/50 text-sm font-medium">Arraste o vídeo aqui ou clique para procurar</p>
                  <p className="text-xs text-white/40">MP4, MOV, AVI — qualquer tamanho (áudio extraído se &gt;25MB)</p>
                </div>
              )}
            </div>
            ...
            <div className="relative flex items-center">
              ...
              <span className="px-3 text-xs text-white/40 uppercase tracking-wider">ou cole a URL</span>
              <div className="flex-1 h-px bg-white/[0.06]" />
            </div>

            <div className="relative">
              <LinkIcon className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-white/40" />
              <input value={videoUrl} onChange={e => setVideoUrl(e.target.value)} placeholder="https://www.tiktok.com/@marca/video/..."
